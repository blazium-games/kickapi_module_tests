...
func test_004_fetch_channels_api():
	if secrets.is_empty():
	...
	# Request channel details directly via dictionary
	var params = {"slug": "xqc"}
	KickAPI.get_channels().get_channels(params)
	...
	var cb = func(sig, code, data):
		print("DEBUG_SIG: ", sig, " CODE: ", code)
		if sig == "channels_received":
			signal_state[0] = true
	...
	if response_payload != null:
		...
		assert_eq(sig_name, "channels_received", "Signal name mapping channels extraction.")
		assert_true(response_code == 200 or response_code == 201 or response_code == 401 or response_code == 404, "HTTP OK should be cleanly fired.")
		assert_not_null(response_data, "Payload must not be empty.")
